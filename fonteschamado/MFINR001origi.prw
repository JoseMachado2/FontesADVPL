...
Static Function ReportPrint(oReport,aOrdem,cAliasTop)
	...
	cQuery += " AND E5_SITUACA NOT IN ('C','E','X') AND E5_TIPODOC NOT IN ('DC','D2','JR','J2','TL','MT','M2','CM','C2','BA') "
	...
	cQuery += " AND E8_FILIAL BETWEEN '"+MV_PAR03+"' AND '"+MV_PAR04+"'  AND E8_DTSALAT < '"+DtoS(MV_PAR05)+"'   "
	...
	Do While !oReport:Cancel() .And. !TE5->(Eof())
        ...
        //cBanco  := TE5->BANCO
		cFilInfo := TE5->EMPRESA
        nRec    := 0
        nDesp   := 0

        While !oReport:Cancel() .And. !TE5->(Eof()) .And. cFilInfo == TE5->EMPRESA
        ...
        oRelBCO:Cell('Saldo Atual'):SetValue(nIni-nDesp+nRec)
...
Static Function CriaCab( oRelatorio )
	...
	aCabec := {	"__LOGOEMP__" + "         " + cChar + "         " + RptFolha+TRANSFORM(oRelatorio:Page(),'999999');
		, Padc(UPPER("Relatório Consolidado de Contas - ") + FWFilialName(_cEmp),132);
		, Padc("",132);
